...
parser.add_argument('-r',action='store',dest='region',required=True,help='Required: Target region [us-east-1,us-east-2,eu-west-1]. ')
...
def registerNUpdateTask(mode,confFile,region):
    """Function to parse config file for registering and creating/updating task def"""
    client = boto3.client('ecs',region_name=region)
    with open(confFile, 'r') as config:
         confParser =  yaml.load(config)
    # keeping failure to 503 since no error code is returned
    ...
    for modules in confParser['containers']:
        ...
        try:
           ...
           networkMode = 'bridge'
        try:
            executionRoleArn = modules['executionRoleArn']
        except:
            executionRoleArn = ''
        # verbose
        #if mode:
        ...
        try:
           if mode:
              print "registering/updating task def: %s " % (family)
           if volumes:
              response = client.register_task_definition(executionRoleArn=executionRoleArn, family=family, networkMode=networkMode, containerDefinitions=containerDefinitions, volumes=volumes)
           else:
              response = client.register_task_definition(executionRoleArn=executionRoleArn, family=family, networkMode=networkMode, containerDefinitions=containerDefinitions)
...
def createService(mode,confFile,region,count,clusterName):
    """Function to parse config file for registering and creating services"""
    client = boto3.client('ecs',region_name=region)
    with open(confFile, 'r') as config:
         confParser =  yaml.load(config)
    # keeping failure to 503 since no error code is returned
       ...
       for modules in confParser['containers']:
           for svc in modules['serviceCreate']:
               ...
               try:
                 ...
                 svcloadBalancers = []
               try:
                   svcNetworkConfiguration = svc['networkConfiguration']
               except:
                   svcNetworkConfiguration = {}
                   svcLbrole = ''
               # verbose
               #if mode:
                  #pprint.pprint(svc)
               try:
                  ...
                  # If task def was registered then create service
                  if len(svcNetworkConfiguration) == 0:
                     ...
                     svcCreateResponse = client.create_service(cluster=svcCluster,serviceName=svcServiceName,taskDefinition=svcTaskDefinition,loadBalancers=svcloadBalancers,networkConfiguration=svcNetworkConfiguration,desiredCount=svcDesiredCount,clientToken=svcClientToken,deploymentConfiguration=svcdeploymentConfiguration)
