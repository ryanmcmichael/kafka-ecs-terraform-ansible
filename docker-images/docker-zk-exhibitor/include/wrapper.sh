...
# fixing hostname for weave to be IP address 
# HOSTNAME=`awk '/^[[:space:]]*($|#)/{next} /'$hostname'/{print $1; exit}' /etc/hosts`
#HOSTNAME=$(uuidgen)

HOSTNAME="zk"-$(echo $(( RANDOM % (10 - 5 + 1 ) + 5 )))

MISSING_VAR_MESSAGE="must be set"
DEFAULT_AWS_REGION="eu-west-1"
DEFAULT_DATA_DIR="/opt/zookeeper/snapshots"
DEFAULT_LOG_DIR="/opt/zookeeper/transactions"
DEFAULT_ZK_ENSEMBLE_SIZE=3
S3_SECURITY=""
HTTP_PROXY=""
...
cat <<- EOF > /opt/exhibitor/defaults.conf
	...
	election-port=3888
	zoo-cfg-extra=tickTime\=2000&initLimit\=10&syncLimit\=5&quorumListenOnAllIPs\=false
